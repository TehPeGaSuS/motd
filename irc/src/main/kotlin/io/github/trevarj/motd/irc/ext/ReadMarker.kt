...
internal object ReadMarkerCommands {
    ...
    fun set(target: String, timestampMs: Long): IrcMessage = set("MARKREAD", target, timestampMs)
    ...
    fun set(command: String, target: String, timestampMs: Long): IrcMessage =
        IrcMessage(
            command = command,
    ...
    fun get(command: String, target: String): IrcMessage =
        IrcMessage(command = command, params = listOf(target))
}