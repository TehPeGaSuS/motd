...
internal sealed interface ParsedDcc {
    data class Send(
        val offer: IrcEvent.DccSendOffer,
    ) : ParsedDcc

    data class Resume(
        val request: IrcEvent.DccResumeRequest,
    ) : ParsedDcc

    data class Accept(
        val accepted: IrcEvent.DccResumeAccepted,
    ) : ParsedDcc
...
private fun parseDccSend(
    ...
    val filename =
        validDccText(tokens[2], maxLength = 255)
            ?: return ParsedDcc.Unsupported(command, IrcEvent.DccUnsupportedReason.MALFORMED)
    val endpoint =
        parseDccEndpoint(tokens[3], tokens[4], token = tokens.getOrNull(6))
            ?: return ParsedDcc.Unsupported(command, IrcEvent.DccUnsupportedReason.MALFORMED)
    val size =
        tokens.getOrNull(5)?.parseDccNonNegativeLong()
            ?: if (tokens.size >= 6) {
                return ParsedDcc.Unsupported(
                    command,
    ...
    val token =
        parseOptionalDccToken(tokens.getOrNull(6))
            ?: if (tokens.size >= 7) {
                return ParsedDcc.Unsupported(
                    command,
                    IrcEvent.DccUnsupportedReason.MALFORMED,
...
private fun parseDccResume(tokens: List<String>): ParsedDcc {
    ...
    val filename =
        validDccText(tokens[2], maxLength = 255)
            ?: return ParsedDcc.Unsupported("RESUME", IrcEvent.DccUnsupportedReason.MALFORMED)
    val port =
        parseDccPort(tokens[3], token = tokens.getOrNull(5))
            ?: return ParsedDcc.Unsupported("RESUME", IrcEvent.DccUnsupportedReason.MALFORMED)
    val position =
        tokens[4].parseDccNonNegativeLong()
            ?: return ParsedDcc.Unsupported("RESUME", IrcEvent.DccUnsupportedReason.MALFORMED)
    val token =
        parseOptionalDccToken(tokens.getOrNull(5))
            ?: if (tokens.size >= 6) {
                return ParsedDcc.Unsupported(
                    "RESUME",
                    IrcEvent.DccUnsupportedReason.MALFORMED,
...
private fun tokenizeDcc(payload: String): Result<List<String>> =
    runCatching {
...
private fun String.parseDccNonNegativeLong(): Long? = takeIf { it.isNotEmpty() && it.all(Char::isDigit) }?.toLongOrNull()

private fun parseOptionalDccToken(value: String?): String? = value?.let { validDccText(it, maxLength = 128)?.takeIf(String::isNotEmpty) }

private fun validDccText(
    value: String,
    maxLength: Int,
): String? = value.takeIf { it.isNotEmpty() && it.length <= maxLength && it.none(Char::isISOControl) }
