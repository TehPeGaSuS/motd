...
/** Deterministic, dependency-free generated-test runner with one independently replayable RNG per case. */
    ...
    private fun writeFailure(
        ...
        File(dir, "${target.safe()}-$seedHash-${fuzzCase.index}.txt").writeText(
            buildString {
                ...
                fuzzCase.trace().forEachIndexed { index, operation ->
                    appendLine("$index\t$operation")
                }
            },
        )
    }
