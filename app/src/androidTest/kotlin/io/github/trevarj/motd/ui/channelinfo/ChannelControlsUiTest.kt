...
class ChannelControlsUiTest {
    ...
    @Test
    fun limitDialog_presetChipFillsTheFieldAndConfirmSendsIt() {
        var limit: Int? = null
        compose.setContent { Controls(catalog = ModeCatalog.DEFAULT, onSetLimit = { limit = it }) }

        compose.onNodeWithTag("channelinfo_limit_set").performScrollTo().performClick()
        compose.onNodeWithTag("channelinfo_limit_chip_50").performClick()
        compose.onNodeWithTag("channelinfo_limit_input").assertTextContains("50")
        compose.onNodeWithTag("channelinfo_limit_confirm").performClick()

        compose.runOnIdle { assertEquals(50, limit) }
    }
