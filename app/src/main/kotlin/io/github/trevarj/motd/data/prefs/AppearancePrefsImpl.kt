...
class AppearancePrefsImpl @Inject constructor(
    ...
    override val config: Flow<AppearanceConfig> = store.data.map { prefs ->
        ...
        AppearanceConfig(
            ...
            fontChoice = prefs[FONT_CHOICE]?.let { runCatching { FontChoice.valueOf(it) }.getOrNull() }
                ?: FontChoice.SYSTEM,
            showTimestamps = prefs[SHOW_TIMESTAMPS] ?: true,
            timeFormat = prefs[TIME_FORMAT]?.let { runCatching { TimeFormat.valueOf(it) }.getOrNull() }
                ?: TimeFormat.AUTO,
            messageSpacing = prefs[MESSAGE_SPACING]?.let { runCatching { MessageSpacing.valueOf(it) }.getOrNull() }
                ?: MessageSpacing.DEFAULT,
            bubbleCornerStyle = prefs[BUBBLE_CORNER_STYLE]
                ?.let { runCatching { BubbleCornerStyle.valueOf(it) }.getOrNull() }
                ?: BubbleCornerStyle.ROUNDED,
            launcherIcon = prefs[LAUNCHER_ICON]?.let { runCatching { LauncherIcon.valueOf(it) }.getOrNull() }
