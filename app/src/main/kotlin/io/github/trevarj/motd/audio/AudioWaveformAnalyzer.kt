...
@Singleton
class AudioWaveformAnalyzer @Inject constructor(
    @IoDispatcher private val ioDispatcher: CoroutineDispatcher,
) {
    suspend fun analyze(file: File): AudioWaveform? = withContext(ioDispatcher) {
                ...
                when (val outputIndex = decoder.dequeueOutputBuffer(info, DEQUEUE_TIMEOUT_US)) {
                    ...
                    MediaCodec.INFO_TRY_AGAIN_LATER -> Unit
                        ...
                        decoder.getOutputBuffer(outputIndex)?.let { output ->
                            ...
                            if (durationUs != null) {
                                val bin = ((info.presentationTimeUs * peaks.size) / durationUs)
                                    .toInt()
    ...
    private fun peakAmplitude(buffer: java.nio.ByteBuffer, encoding: Int): Int = when (encoding) {
