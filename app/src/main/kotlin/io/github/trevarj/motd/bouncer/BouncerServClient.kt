...
sealed interface BouncerServResult {
    ...
    data class Success(
        override val displayCommand: String,
...
@Singleton
class ConnectionBouncerServSessionProvider
    @Inject
    constructor(
    ...
    constructor(
        ...
        override suspend fun execute(
            ...
            locks.getOrPut(rootNetworkId) { Mutex() }.withLock {
                val session =
                    sessions.session(rootNetworkId)
                ...
                try {
                    coroutineScope {
                        ...
                        try {
                            ...
                            val completed =
                                withTimeoutOrNull(HARD_LIMIT_MS) {
                                    val first =
                                        withTimeoutOrNull(FIRST_REPLY_MS) { replies.receive() }
                            ...
                            if (completed) {
                                BouncerServResult.Success(command.display, lines)
                            } else {
                                BouncerServResult.Timeout(command.display, lines)
                            }
                        } finally {
                            collector.cancel()
                            replies.close()
...
fun parseAvailableCommandPaths(
    lines: List<String>,
    family: String? = null,
): Set<String> {
