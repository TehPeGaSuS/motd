...
@Singleton
class DataStorePushHealthStore
    @Inject
    constructor(
        ...
        override suspend fun verifying(networkId: Long) =
            update(networkId) { old ->
                old.copy(
        ...
        override suspend fun registered(networkId: Long) =
            update(networkId) { old ->
                old.copy(
                    capability = PushCapability.SUPPORTED,
        ...
        override suspend fun probeDelivered(networkId: Long) =
            update(networkId) { old ->
                old.copy(probeAt = System.currentTimeMillis(), errorCode = null, errorAt = null)
            }

        override suspend fun messageDelivered(networkId: Long) =
            update(networkId) { old ->
        ...
        private suspend fun update(
            networkId: Long,
            transform: (NetworkPushHealth) -> NetworkPushHealth,
        ) {
            store.edit { prefs ->
                val current = decode(prefs[key]).toMutableMap()
        ...
        private fun write(
            ...
            value: Map<Long, NetworkPushHealth>,
        ) {
                    ...
                    .mapNotNull { (idText, element) ->
                        ...
                        id to
                            NetworkPushHealth(
                                ...
                                capability =
                                    obj.string("capability")?.let { runCatching { PushCapability.valueOf(it) }.getOrNull() }
        ...
        private fun encode(value: Map<Long, NetworkPushHealth>): String =
            buildJsonObject {
