...
internal fun decodeAgentwireRecents(raw: String?): List<AgentwireRecentSession> =
...
internal fun encodeAgentwireRecents(sessions: List<AgentwireRecentSession>): String = recentsJson.encodeToString(sessions)
...
): List<AgentwireRecentSession> =
    (listOf(session) + current.filterNot { it.sid == session.sid })
...
private fun drawerRow(
    ...
    val blocked =
        flags.any { it.startsWith("waiting", ignoreCase = true) } ||
