...
class AvatarPrefsImpl
    ...
    constructor(
        ...
        override val config: Flow<AvatarConfig> =
            store.data.map { prefs ->
                AvatarConfig(showSharedAvatars = prefs[SHOW_SHARED] ?: true)
            }

        override fun selfSetting(networkId: Long): Flow<SelfAvatarSetting> =
            store.data.map { prefs ->
...
internal fun encodeSelfSetting(setting: SelfAvatarSetting): String =
    when (setting) {
...
internal fun decodeSelfSetting(value: String?): SelfAvatarSetting =
    when {
