...
class LinkPreviewRepositoryImpl @Inject constructor(
    ...
    override fun cachedPreview(url: String, networkId: Long?): CachedLinkPreview? =
        synchronized(cache) {
            cache.get(cacheKey(url, networkId))?.let { CachedLinkPreview(it.value) }
        }

    override suspend fun preview(url: String, networkId: Long?): LinkPreview? {
    ...
    private suspend fun fetchRouted(url: String, networkId: Long?): LinkPreview? {
    ...
    private suspend fun fetch(url: String, route: NetworkMediaRoute): LinkPreview? =
    ...
    ): LinkPreview? = request(summaryUrl, WIKIPEDIA_ACCEPT, route, connection) { conn ->
    ...
    private fun fetchGenericPreview(
        ...
        route: NetworkMediaRoute,
        connection: AtomicReference<HttpURLConnection?>,
    ): LinkPreview? = request(url, GENERIC_ACCEPT, route, connection) { conn ->
        val contentType = conn.getHeaderField("Content-Type")
    ...
    private fun <T> request(
        ...
        while (true) {
            ...
            val conn = route.open(current).apply {
                requestMethod = "GET"
            ...
            try {
                ...
                when {
                    code in 200..299 -> return read(conn)
    ...
    companion object {
        ...
        private fun cacheKey(url: String, networkId: Long?): String = "$networkId|$url"
         ...
         */
        internal fun isAllowedDestination(url: URL, resolveDns: Boolean): Boolean {
            if (!url.protocol.equals("https", ignoreCase = true)) return false
            val host = url.host.orEmpty().removePrefix("[").removeSuffix("]").trimEnd('.')
        ...
        private fun Regex.firstGroup(html: String): String? {
            ...
            return (m.groupValues.getOrNull(1)?.takeIf { it.isNotEmpty() }
                ...
                ?.let(::decodeEntities)?.let(::sanitizeText)?.trim()?.takeIf { it.isNotEmpty() }
        }
        ...
        fun parseOgTags(url: String, html: String): LinkPreview? {
            ...
            val title = OG_TITLE.firstGroup(head)
                ?: TITLE_TAG.find(head)?.groupValues?.getOrNull(1)
                    ?.let(::decodeEntities)?.let(::sanitizeText)?.trim()?.takeIf { it.isNotEmpty() }
