...
fun BouncerServResult.safeSummary(): String =
    when (this) {
        is BouncerServResult.Success -> {
            replies.lastOrNull()?.let(::redactBouncerServReply)
                ?: "Command completed"
        }

        is BouncerServResult.Timeout -> {
            if (replies.isEmpty()) {
        ...
        is BouncerServResult.Disconnected -> {
            "Connect the soju account to run commands"
        }

        is BouncerServResult.Failed -> {
            "BouncerServ command failed"
        }
    }

fun extractUserDeletionToken(
    username: String,
    replies: List<String>,
): String? {
    val expected =
...
fun parseChannelStatus(replies: List<String>): List<BouncerChannelRow> =
    replies.mapNotNull { line ->
