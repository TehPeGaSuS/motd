...
fun redactBouncerServCommand(raw: String): String {
    ...
    when (path) {
        "sasl set-plain" -> {
            if (out.isNotEmpty()) out[out.lastIndex] = REDACTED
        }

        "user delete" -> {
            if (out.size > 3) out[3] = REDACTED
        }
...
fun redactBouncerServReply(raw: String): String {
    ...
    val looksSensitiveFailure =
        (lower.startsWith("error") || lower.startsWith("fail")) &&
