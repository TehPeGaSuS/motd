...
class ConnectionManagerOnboardingBouncerOperations
    @Inject
    constructor(
        ...
        override suspend fun list(rootNetworkId: Long): List<BouncerNetwork> = client(rootNetworkId).bouncerListNetworks()

        override suspend fun add(
            rootNetworkId: Long,
            name: String,
            host: String,
        ): String = client(rootNetworkId).bouncerAddNetwork(mapOf("name" to name, "host" to host))

        private fun client(rootNetworkId: Long): IrcClient =
            connectionManager
                .clientFor(rootNetworkId)
...
internal fun bouncerOperationError(error: Throwable): BouncerOperationError =
    when (error) {
        is IrcDisconnectedException -> {
            BouncerOperationError.ConnectionLost
        }

        is IrcCommandException -> {
            BouncerOperationError.ServerRejected(
                error.text.replace(Regex("[\\r\\n]+"), " ").trim(),
            )
        }

        else -> {
            BouncerOperationError.Unexpected(
                error.message
                    .orEmpty()
                    .replace(Regex("[\\r\\n]+"), " ")
                    .trim(),
