...
class ReplyReceiver : BroadcastReceiver() {
    ...
    private suspend fun send(bufferId: Long, text: String, retry: Boolean) = deliverNotificationReply(
        retry = retry,
    ...
    private suspend fun preserveRejectedDraft(bufferId: Long, text: String) {
        runCatching {
    ...
    private suspend fun clearRetriedDraft(bufferId: Long, text: String) {
        runCatching {
...
internal suspend fun deliverNotificationReply(
    ...
    when (val acceptance = send()) {
        is SendAcceptance.Accepted -> if (retry) {
            releaseDraft()
            notifyResolved()
        }
...
internal fun mergeRejectedReply(existing: String?, rejected: String): String =
    if (existing.isNullOrBlank()) rejected else "$existing\n$rejected"
 ...
 */
internal fun withoutRetriedReply(existing: String?, retried: String): String? = when {
    existing == null -> null
    existing == retried -> ""
