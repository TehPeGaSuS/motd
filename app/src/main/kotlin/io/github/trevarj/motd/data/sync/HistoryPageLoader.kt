...
class HistoryPageLoader @Inject constructor(
     ...
     */
    internal class LatestFlightTimeoutException :
        Exception("CHATHISTORY LATEST request timed out")
    ...
    suspend fun loadPage(
        ...
        return when (direction) {
            ...
            Direction.LATEST -> try {
                fetchLatest(
                    networkId, roomId, target, source, requestLimit, referenceTypes,
                    requestTimeoutMs, allowConcurrent = allowConcurrent,
                ).toPageResult()
            ...
            Direction.OLDER -> coalesced(FlightKey(networkId, roomId, direction, gapId)) {
                loadOlder(
                    networkId, roomId, target, source, requestLimit, referenceTypes, gapId, boundary,
                    allowConcurrent,
                )
            }
            Direction.NEWER -> coalesced(FlightKey(networkId, roomId, direction, gapId)) {
                loadNewer(
                    networkId, roomId, target, source, requestLimit, referenceTypes, gapId, boundary,
                    allowConcurrent,
                )
            ...
            Direction.AROUND -> PageResult.Failed(
    ...
    suspend fun loadAround(
        ...
        val anchor = msgidSelector ?: timestampSelector ?: return PageResult.Failed(
    ...
    ): Pair<ChatHistoryRequest, ChatHistoryResponse> = try {
    ...
    internal suspend fun fetchLatest(
    ...
    ): LatestPage = coalesced(FlightKey(networkId, roomId, Direction.LATEST, gapId = null)) {
        ...
        val result = try {
            fetchMessages(
                networkId, source, request, referenceTypes, allowMsgid, timeoutMs,
                retryableTimeout = false, allowConcurrent = allowConcurrent,
            )
        } catch (_: TimeoutCancellationException) {
    ...
    private fun LatestPage.toPageResult(): PageResult {
        ...
        return PageResult.Loaded(
            ...
            inserted,
            endOfDirection = response.isComplete ||
...
internal fun IrcClient.historySource(): HistoryPageLoader.HistorySource =
    object : HistoryPageLoader.HistorySource {
        ...
        override suspend fun chathistory(req: ChatHistoryRequest): ChatHistoryResponse =
            this@historySource.chathistory(req)
    }
