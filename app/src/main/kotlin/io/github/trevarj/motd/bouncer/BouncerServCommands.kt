...
object BouncerServCommands {
    ...
    fun networkUpdate(
        name: String,
        changed: NetworkCommandFields,
    ): BouncerServCommand =
        BouncerServCommand(
            buildList {
                add("network")
                add("update")
                add(quoteBouncerArg(name))
    ...
    fun channelStatus(network: String) =
        BouncerServCommand(
    ...
    fun channelCreate(
        channel: String,
        network: String,
        fields: ChannelCommandFields = ChannelCommandFields(),
    ...
    fun accountUpdate(
        ...
        buildList {
            add("user")
            add("update")
            nick?.takeIf(String::isNotBlank)?.let {
                add("-nick")
                add(quoteBouncerArg(it))
            }
            realName?.takeIf(String::isNotBlank)?.let {
                add("-realname")
                add(quoteBouncerArg(it))
            }
        }.joinToString(" "),
    ...
    private fun rootNetworkCommand(
        path: String,
        network: String,
    ) = BouncerServCommand(
    ...
    private fun channelMutation(
        ...
        buildList {
            ...
            add("channel")
            add(operation)
            add(quoteBouncerArg("$channel/$network"))
            fields.detached?.let {
                add("-detached")
                add(it.toString())
            }
            fields.relayDetached?.let {
                add("-relay-detached")
                add(quoteBouncerArg(it))
            }
            fields.reattachOn?.let {
                add("-reattach-on")
                add(quoteBouncerArg(it))
            }
            fields.detachAfter?.let {
                add("-detach-after")
                add(quoteBouncerArg(it))
            }
            fields.detachOn?.let {
    ...
    private fun MutableList<String>.appendNetworkFields(
        fields: NetworkCommandFields,
        ...
        fields.connectCommands?.forEach {
            add("-connect-command")
            add(quoteBouncerArg(it))
        }
    }
