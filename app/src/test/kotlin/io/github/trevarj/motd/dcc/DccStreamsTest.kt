...
class DccStreamsTest {
    @Test fun `receive streams declared bytes and writes acknowledgements`() = runTest {
        val output = ByteArrayOutputStream()
        val ack = ByteArrayOutputStream()
        val progress = mutableListOf<Long>()

        val total = receiveDccBytes(
    ...
    @Test fun `declared size truncation fails after preserving partial progress`() = runTest {
        val output = ByteArrayOutputStream()
        val progress = mutableListOf<Long>()

        val error = expectIntegrityFailure {
            receiveDccBytes(
    ...
    @Test fun `unknown size transfer fails when it exceeds safety limit`() = runTest {
        val output = ByteArrayOutputStream()
        val progress = mutableListOf<Long>()
