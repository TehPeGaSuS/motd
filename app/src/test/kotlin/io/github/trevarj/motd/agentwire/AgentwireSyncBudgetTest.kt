...
class AgentwireSyncBudgetTest {
    ...
    @Test
    fun `silence ends at the budget rather than one backoff step later`() = runTest {
    ...
    @Test
    fun `an internal restart reuses the deadline and honours the send floor`() = runTest {
        val budget = AgentwireSyncBudget(clock).also { it.anchor() }
        val sends = mutableListOf<Long>()
        var ready = false
        ...
        retryAgentwireSync(
            budget = budget,
            isReady = { ready },
            issue = { sends += now; ready = sends.size == 2; true },
            nextId = { "sync-${sends.size}" },
            pause = { now += it },
        )
    ...
    @Test
    fun `three consecutive failed writes end the handshake well inside the budget`() = runTest {
        val budget = AgentwireSyncBudget(clock).also { it.anchor() }
        var consecutive = 0
        var timedOut = 0
    ...
    @Test
    fun `re-anchoring after a failure grants a fresh budget`() = runTest {
        val budget = AgentwireSyncBudget(clock).also { it.anchor() }
