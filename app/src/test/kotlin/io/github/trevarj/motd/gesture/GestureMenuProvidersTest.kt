...
class GestureMenuProvidersTest {
    ...
    @Test fun `pinned chats become open-chat leaves in chat-list order`() = runTest {
        val world = world()
    ...
    @Test fun `unread chats skip muted, server and read rows`() = runTest {
        val world = world()
        world.buffers.chats.value = listOf(
    ...
    @Test fun `a ring never grows past the provider's clamped limit`() = runTest {
        val world = world()
    ...
    @Test fun `recent DMs open by identity rather than by a stored room id`() = runTest {
        val world = world()
        world.buffers.queryConversations.value = listOf(
    ...
    @Test fun `friends come online first and offline ones land on the first connected network`() = runTest {
        val world = world()
        ...
        world.connections.presence.value = mapOf(
    ...
    @Test fun `friends vanish when there is nowhere to message them`() = runTest {
        val world = world()
    ...
    @Test fun `networks offer the flip their current state allows`() = runTest {
    ...
    @Test fun `a provider kind from a newer build resolves to an empty ring`() = runTest {
        val world = world()
    ...
    @Test fun `resolved leaf ids stay unique inside one ring`() = runTest {
        val world = world()
    ...
    ): GestureNode.Provider = GestureNode.Provider(
        id = "ring",
        label = kind.name,
