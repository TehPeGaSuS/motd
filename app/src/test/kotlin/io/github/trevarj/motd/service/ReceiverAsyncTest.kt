...
class ReceiverAsyncTest {
    @Test
    fun `successful work finishes exactly once`() =
        runTest {
            var finishes = 0
    ...
    @Test
    fun `failure is reported and finishes exactly once`() =
        runTest {
    ...
    @Test
    fun `timeout cancels work and finishes exactly once under virtual time`() =
        runTest {
            var timedOut = 0
            var finishes = 0
            val job =
    ...
    @Test
    fun `parent cancellation still finishes and propagates`() =
        runTest {
            var finishes = 0
