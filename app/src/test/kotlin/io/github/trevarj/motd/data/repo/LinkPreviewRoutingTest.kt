...
class LinkPreviewRoutingTest {
    ...
    @Test
    fun proxy_error_fails_closed_without_a_direct_fallback() = runTest {
        val resolver = MediaRouteResolver { id -> route(id, proxyError = "local SOCKS unavailable") }
        val repository = LinkPreviewRepositoryImpl(
            prefs, resolver, relaxed, this, StandardTestDispatcher(testScheduler),
        )
    ...
    @Test
    fun unknown_network_identity_fetches_nothing() = runTest {
        val repository = LinkPreviewRepositoryImpl(
            prefs, { id -> route(id) }, relaxed, this, StandardTestDispatcher(testScheduler),
        )
        server.enqueue(htmlResponse("Leak"))
    ...
    @Test
    fun missing_route_fetches_nothing() = runTest {
        val repository = LinkPreviewRepositoryImpl(
            prefs, { _ -> null }, relaxed, this, StandardTestDispatcher(testScheduler),
        )
        server.enqueue(htmlResponse("Leak"))
    ...
    @Test
    fun fetch_traverses_the_route_proxy_instead_of_connecting_directly() = runBlocking {
    ...
    @Test
    fun route_authorization_is_never_attached_to_a_preview_request() = runTest {
    ...
    @Test
    fun negative_results_are_scoped_to_the_network_that_produced_them() = runTest {
        ...
        val resolver = MediaRouteResolver { id ->
    ...
    @Test
    fun overall_deadline_bounds_a_byte_dripping_server() = runBlocking {
    ...
    @Test
    fun in_flight_fetches_are_bounded_by_the_permit_pool() = runBlocking {
