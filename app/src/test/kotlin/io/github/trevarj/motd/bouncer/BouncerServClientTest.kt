...
class BouncerServClientTest {
    @Test fun ignores_notice_and_finishes_after_privmsg_quiet_window() =
        runTest {
            val events = MutableSharedFlow<IrcEvent>(extraBufferCapacity = 8)
            val provider =
                FakeSessions(events) { wire ->
    ...
    @Test fun first_reply_timeout_is_retryable_and_stale_session_replies_are_discarded() =
        runTest {
            val events = MutableSharedFlow<IrcEvent>(extraBufferCapacity = 2)
            var current = true
            val provider =
                FakeSessions(events, isCurrent = { current }) {
                    current = false
    ...
    @Test fun commands_for_one_root_are_serialized() =
        runTest {
            val events = MutableSharedFlow<IrcEvent>(extraBufferCapacity = 8)
    ...
    @Test fun known_silent_command_completes_after_write_without_inventing_state() =
        runTest {
            val events = MutableSharedFlow<IrcEvent>()
            val sent = mutableListOf<String>()
            val result =
                BouncerServClientImpl(FakeSessions(events) { sent += it })
    ...
    @Test fun durable_but_disconnected_command_does_not_report_success_or_timeout() =
        runTest {
            val events = MutableSharedFlow<IrcEvent>()
            val provider =
                FakeSessions(
                    events = events,
                    acceptance =
                        io.github.trevarj.motd.service.SendAcceptance.Accepted(
                            emptyList(),
            ...
            val result =
                BouncerServClientImpl(provider)
                    .execute(1, BouncerServCommands.serverDebug(true))

            assertEquals(BouncerServResult.Disconnected("server debug true"), result)
        }

    @Test fun durable_but_failed_wire_command_reports_failure_immediately() =
        runTest {
            val events = MutableSharedFlow<IrcEvent>()
            val provider =
                FakeSessions(
    ...
    @Test fun probe_expands_advertised_command_families() =
        runTest {
                ...
                FakeSessions(events) { wire ->
                    val replies =
                        when (wire) {
                            ...
                            "help network" -> {
                                listOf("available commands: network create, network status")
                            }

                            "help server" -> {
                                listOf("available commands: server status, server debug")
                            }
    ...
    private class FakeSessions(
        ...
        override suspend fun session(rootNetworkId: Long) =
            BouncerServSession(
                token = this,
                events = events,
                send = {
