...
class PresetEnrollmentCoordinatorTest {
    ...
    private fun libera(role: NetworkRole = NetworkRole.DIRECT) =
        NetworkEntity(
            id = 7,
            name = "Libera.Chat",
            role = role,
    ...
    @Test
    fun eligible_first_ready_sends_once_and_reconnect_does_not_repeat() =
        runTest {
    ...
    @Test
    fun failed_write_is_attempted_and_never_retried() =
        runTest {
            val prefs = FakePrefs().apply { markLiberaEligible(7) }
            val coordinator = PresetEnrollmentCoordinator(prefs, FakeNetworks(libera()))
            var attempts = 0
    ...
    @Test
    fun current_row_must_still_be_a_direct_tls_libera_endpoint() =
        runTest {
            val invalidRows =
                listOf(
    ...
    @Test
    fun an_unmarked_existing_or_imported_row_is_not_inferred_from_its_hostname() =
        runTest {
            val joins = mutableListOf<String>()
    ...
    @Test
    fun an_obsolete_ready_generation_does_not_consume_eligibility() =
        runTest {
            val prefs = FakePrefs().apply { markLiberaEligible(7) }
            val coordinator = PresetEnrollmentCoordinator(prefs, FakeNetworks(libera()))
            val joins = mutableListOf<String>()
