...
class InviteJoinCoordinatorTest {
    @Test fun `concurrent stale action deliveries send exactly one join`() =
        runTest {
            val lock = Mutex()
    ...
    @Test fun `timeout has no delayed side effect and remains retryable`() =
        runTest {
            var state = InviteState.PENDING
            var sends = 0

            suspend fun attempt(ready: Boolean) =
                performInviteJoin(
                    initialState = state,
                    claim = { from ->
                        if (state == from) {
    ...
    @Test fun `dismiss during connection wait prevents wire write`() =
        runTest {
            var state = InviteState.PENDING
            var sends = 0
