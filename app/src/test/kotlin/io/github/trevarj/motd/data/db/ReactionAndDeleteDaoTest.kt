...
class ReactionAndDeleteDaoTest {
    ...
    @Test
    fun observeForBuffer_returnsAllReactionsWithoutAnInList() =
        runTest {
    ...
    @Test
    fun optimisticOwnReaction_reconcilesWithServerEcho_withoutDuplicating() =
        runTest {
            val dao = db.reactionDao()
    ...
    @Test
    fun actorCanRetainMultipleEmojis_andDeleteIsReactionSpecific() =
        runTest {
            val dao = db.reactionDao()
            dao.upsert(ReactionEntity(bufferId = bufferId, targetMsgid = "m1", actorKey = "nick:me", sender = "me", emoji = "👍", serverTime = 1))
            dao.upsert(ReactionEntity(bufferId = bufferId, targetMsgid = "m1", actorKey = "nick:me", sender = "me", emoji = "🎉", serverTime = 2))
    ...
    @Test
    fun observeMsgid_isNullWhilePending_thenEmitsMsgidOnceEchoLands() =
        runTest {
    ...
    @Test
    fun deleteById_removesTheFailedRow() =
        runTest {
            val dao = db.messageDao()
            val id =
                dao
