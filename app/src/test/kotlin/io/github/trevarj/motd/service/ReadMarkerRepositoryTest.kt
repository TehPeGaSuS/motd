...
class ReadMarkerRepositoryTest {
    ...
    @Test
    fun `bulk boundary is newest incoming chat and ignores unsafe newer rows`() =
        runTest {
    ...
    @Test
    fun `stored snapshot includes unset markers but excludes server buffers`() =
        runTest {
    ...
    @Test
    fun `reconnect candidate comes from local anchor and never lowers known remote marker`() =
        runTest {
    ...
    @Test
    fun `future pending anchor resolves wire marker to newest authoritative chat`() =
        runTest {
            val room = db.bufferDao().insert(buffer(networkId, "#motd"))
            val authoritativeId =
                db
    ...
    @Test
    fun `marker waiter subscribes before request and selects matching target`() =
        runTest {
