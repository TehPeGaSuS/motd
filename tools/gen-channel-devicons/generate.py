#!/usr/bin/env python3
"""Generate ChannelDeviconCatalog.kt from a pinned devicons/devicon checkout.
...
def main() -> int:
    ...
    out_path = (
        ...
        if len(sys.argv) > 2
            ...
            "../../app/src/main/kotlin/io/github/trevarj/motd/ui/components/ChannelDeviconCatalog.kt",
        )
    )
    ...
    chunks = [marks[i : i + CHUNK] for i in range(0, len(marks), CHUNK)]
    lines = [
        "// GENERATED by tools/gen-channel-devicons/generate.py from devicons/devicon "
        + PIN
        + " - do not edit.",
        ...
        "        " + " + ".join(f"marks{i}()" for i in range(len(chunks))),
        "    }",
    ]
    for i, chunk in enumerate(chunks):
        lines.append("")
        lines.append(f"    private fun marks{i}(): List<CatalogChannelMark> = listOf(")
        for name, aliases, width, height, paths in chunk:
            alias_list = ", ".join(f'"{a}"' for a in aliases)
            lines.append("        CatalogChannelMark(")
            lines.append(f'            markName = "{name}",')
            lines.append(f"            aliases = setOf({alias_list}),")
            lines.append(f"            viewportWidth = {width}f,")
            lines.append(f"            viewportHeight = {height}f,")
            lines.append("            pathData = listOf(")
            for even_odd, d in paths:
                flag = "true" if even_odd else "false"
                lines.append(f"                {flag} to {kotlin_string(d)},")
            lines.append("            ),")
            lines.append("        ),")
        lines.append("    )")
    lines.append("}")
