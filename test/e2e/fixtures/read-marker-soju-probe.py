#!/usr/bin/env python3
"""Two-client soju.im/read (READ command) proof against the local soju fixture.

Forces the soju.im/read-only fallback path by requesting `soju.im/read` and
declining `draft/read-marker`, then exercises set/get/broadcast/persist with
the `READ` command. Mirrors read-marker-probe.py's draft/read-marker smoke so
the two extensions stay at parity. Soju picks the broadcast command per
downstream client based on what it negotiated, so a client that ACKs only
soju.im/read sees READ traffic only.
"""
...
class Client:
    ...
    def close(self) -> None:
        try:
            self.send("QUIT :read-marker soju probe complete")
        except OSError:
            pass
        self.sock.close()
...
def connect(host: str, port: int, username: str, password: str, nick: str) -> Client:
    ...
    client.send(f"USER {nick} 0 * :motd soju read-marker probe")
    ...
    if "soju.im/read" not in offered:
        raise AssertionError(f"soju did not advertise soju.im/read: {offered}")
    # Request soju.im/read and deliberately decline draft/read-marker so soju broadcasts READ.
    client.send("CAP REQ :sasl soju.im/read server-time echo-message message-tags")
    ack = client.wait_for(lambda line: " CAP " in line and " ACK " in line, "CAP ACK")
    if "soju.im/read" not in ack:
        raise AssertionError(f"soju did not ACK soju.im/read: {ack}")
    if "draft/read-marker" in ack:
        raise AssertionError(f"soju ACKed draft/read-marker despite not requesting it: {ack}")
...
def is_marker(line: str, channel: str) -> bool:
    # soju.im/read broadcasts use the READ command, not MARKREAD.
    return " READ " in line and f" {channel} " in line and marker_value(line) is not None


def smoke(host: str, port: int, username: str, password: str, network: str, channel: str) -> None:
    authcid = f"{username}/{network}"
    first = connect(host, port, authcid, password, "motdsojuread1")
    second = connect(host, port, authcid, password, "motdsojuread2")
    reconnect: Client | None = None
    try:
        token = f"soju-read-{time.time_ns()}"
        ...
        first_start = len(first.lines)
        second_start = len(second.lines)
        first.send(f"READ {channel} timestamp={timestamp}")
        ...
        second.send(f"READ {channel} timestamp={older}")
        query_start = len(second.lines)
        second.send(f"READ {channel}")
        ...
        second.close()
        reconnect = connect(host, port, authcid, password, "motdsojuread3")
        query_start = len(reconnect.lines)
        reconnect.send(f"READ {channel}")
        ...
        print("SOJU_READ_MARKER_SMOKE_OK")
...
if __name__ == "__main__":
    try:
        ...
        print(f"SOJU_READ_MARKER_PROBE_FAILED: {error}", file=sys.stderr)
        raise SystemExit(1)